		...
		mutations.forEach(mutation => {
			if (mutation.type === "childList") {
				mutation.addedNodes.forEach(node => {
					if (node.nodeType === 1) {
						if (node.hasAttribute("data-adt-map")) applyEventHandlers(node);
...
function enableRectangleSelection(map, onSelectionChange, showSelectionOrder) {
	...
	map.on('mousemove', (e) => {
		if (isDrawing && rectangle) rectangle.setBounds([startPoint, e.latlng]);
	});
		...
		if (isDrawing) {
			...
			if (rectangle) {
				...
				markerMap.forEach((markerInstance) => {
					if (bounds.contains(markerInstance.getLatLng())) selectedInArea.push(markerInstance);
				});

				const selected = selectedMarkers.get(map);
				selectedInArea.forEach(marker => {
					if (selected.has(marker.options.id)) deselectMarker(marker, map, showSelectionOrder);
					else selectMarker(marker, map, showSelectionOrder);
				});
				...
				if (settings && settings.enabled) calculateRoute(map);
			}
		}
	});
}
	...
	for (const marker of markers) {
		if (marker.img) {
			...
			markerImage.onload = function () {
				...
				loadedCount++;
				if (loadedCount === totalMarkers) checkAndApplyPreselection(map, markers, showSelectionOrder, onSelectionChange, selectable);
			};
		} else {
		...
		if (!marker.excludeFromBoundary) positionsOfMarkers.push(marker.position);
	}

	if (!position.length && positionsOfMarkers.length) map.fitBounds(positionsOfMarkers);
...
function checkAndApplyPreselection(map, markers, showSelectionOrder, onSelectionChange, selectable) {
	if (selectable && showSelectionOrder) applyPreselectedMarkers(map, markers, showSelectionOrder, onSelectionChange);
}
...
function createMarker(marker, options, selectedOptions, cluster = null, selectable = false, onSelectionChange = null, map = null, showSelectionOrder = false, markerInfoCallback = null) {
	...
	if (map) {
		...
		mapMarker.on('add', function () {
			...
			if (el) {
				...
				if (parent) parent.style.pointerEvents = 'all';
			}
		});
	}
	...
	if (selectable && map) {
		mapMarker.on('click', function (e) {
			...
			const selected = selectedMarkers.get(map);
			if (selected.has(marker.id)) deselectMarker(mapMarker, map, showSelectionOrder);
			else selectMarker(mapMarker, map, showSelectionOrder);
			...
			const settings = routeSettingsMap.get(map);
			if (settings && settings.enabled) calculateRoute(map);
			if (originalCallback && window[originalCallback]) window[originalCallback](e);
			L.DomEvent.stopPropagation(e);
		});
	...
	if (marker.popup) mapMarker.bindPopup(marker.popup);
	else if (marker.popupCallback) mapMarker.bindPopup(() => window[marker.popupCallback](mapMarker));

	if (cluster) cluster.addLayer(mapMarker);
...
async function selectMarker(marker, map, showSelectionOrder) {
	...
	if (marker._selectedIcon) marker.setIcon(marker._selectedIcon);
	const icon = marker.getElement();
	if (icon) icon.classList.add('marker-selected');
...
function deselectMarker(marker, map, showSelectionOrder) {
	...
	const icon = marker.getElement();
	if (icon) icon.classList.remove('marker-selected');

	if (showSelectionOrder) {
		const markers = markerInstances.get(map);
		markers.forEach((markerInstance) => {
			const newOrder = order.get(markerInstance.options.id);
			if (newOrder) updateMarkerOrderDisplay(map, markerInstance, newOrder, true);
		});
	}
}
...
async function createMarkerIcon(map, iconUrl, orderNumber, color = null) {
	...
	const settings = routeSettingsMap.get(map);
	if ((settings && settings.enabled) || color) inlineStyle = `--marker-fill: ${color ?? settings.color};`;
...
function applyPreselectedMarkers(map, markersData, showSelectionOrder, onSelectionChange) {
	...
	preselected.forEach((markerData) => {
		...
		if (markerInstance) selectMarker(markerInstance, map, showSelectionOrder);
		else console.warn('Marker instance NOT found for ID:', markerData.id);
	});
	...
	const settings = routeSettingsMap.get(map);
	if (settings && settings.enabled && order.size >= 2) calculateRoute(map);
}
...
async function calculateRoute(map) {
	...
	if (beforeCallback && window[beforeCallback]) window[beforeCallback]();
...
async function calculateRouteMapyCz({ routeMarkers, startPoint, endPoint, routeSettings, allCoords, allParts }) {
	...
	for (let i = 0; i < chunks.length; i++) {
		...
		if (data.geometry?.geometry?.coordinates) allCoords.push(...data.geometry.geometry.coordinates.map(c => [c[1], c[0]]));
		if (data.parts) allParts.push(...data.parts);
	}
}
...
async function calculateRouteHere({ routeMarkers, startPoint, endPoint, routeSettings, allCoords, allParts }) {
	...
	for (let i = 0; i < chunks.length; i++) {
		...
		if (routeSettings.departureTime) params.set('departureTime', routeSettings.departureTime);
...
function clearSelection(mapElement) {
	...
	const order = selectionOrder.get(map);
	const markers = markerInstances.get(map);
	markers.forEach((markerInstance) => { if (selected.has(markerInstance.options.id)) deselectMarker(markerInstance, map, true); });
	selected.clear();
	order.clear();
}
...
function toggleMarker(mapElement, markerId, selected) {
	...
	if (selected && !isSelected) selectMarker(marker, map, true);
	else if (!selected && isSelected) deselectMarker(marker, map, true);
	else return;

	const cluster = markerClusters.get(map);
	if (cluster) cluster.refreshClusters(marker);
...
function setOrder(mapElement, markerId, orderNumber) {
	...
	const markers = markerInstances.get(map);
	const marker = markers?.get(markerId);
	if (settings && settings.route && settings.route.enabled) color = settings.route.color;
	if (!marker) return;
